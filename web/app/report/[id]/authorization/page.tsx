...
export default async function AuthorizationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto max-w-3xl bg-white px-10 py-12 text-ink print:max-w-none">
    ...
    <dl className="my-8 grid grid-cols-[190px_1fr] border-y border-line text-[14px]">
      {[
        ["Organization", a.organizationName], ["Authorized representative", a.authorizedBy],
        ["Contact", a.contactEmail || "Not supplied"], ["Authorized target", a.domain || "No website target"],
        ["Check depth", a.profile], ["Scan reference", id],
    ...
    <div className="mt-12 grid grid-cols-2 gap-12 text-[13px]">
      ...
      <div className="border-t border-ink pt-2">Recorded electronically: {new Date(a.acceptedAt).toLocaleDateString()}</div>
    </div>
    <div className="mt-10 flex gap-3 print:hidden">
      <PrintButton />
      <Link href={`/report/${id}`} className="rounded-xl border border-line px-5 py-3 font-semibold">Back to report</Link>
    </div>
  </main>;
}
